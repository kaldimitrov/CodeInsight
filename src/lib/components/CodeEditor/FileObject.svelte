<script lang="ts">
	import { currentCode, updateCode, updateFile } from './editorStore.js';
	import { FileTypes } from './enums/fileTypes.js';
	export let item: any;

	function handleClick(item: any) {
		if (item.type != FileTypes.FILE) return;

		updateCode(item.content);
		updateFile(item.label);
	}
</script>
...
{:else if item.type === FileTypes.FILE}
	<li>
    <summary class="text-base flex flex-row" on:click={() => handleClick(item)}>
